...
async def getspy(_,CallbackQuery):  
    ...
    universal = InlineKeyboardButton(text="🗑 ᴄʟᴏsᴇ ᴍᴇɴᴜ", callback_data=f'close2')
    if j == 0:
        return await CallbackQuery.message.reply_text("Video Format Not Found..")
    elif j == 1:
        key = InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(text="⬅️  ɢᴏ ʙᴀᴄᴋ", callback_data=f'good {videoid}|{user_id}'),
...
async def ytdata(_,CallbackQuery):  
    ...
    if type == 'audio':
        ...
        a1 = InlineKeyboardButton(text=f"ᴀᴜᴅɪᴏ ꜰᴏʀᴍ", callback_data=f"boom video||{format}||{videoid}")
